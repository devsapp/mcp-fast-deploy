...
class TeaCore:
    ...
    http_adapter = adapters.HTTPAdapter(
        pool_connections=DEFAULT_POOL_SIZE, pool_maxsize=DEFAULT_POOL_SIZE * 4
    )
    ...
    async def async_do_action(request: TeaRequest, runtime_option=None) -> TeaResponse:
        ...
        connect_timeout = (
            runtime_option.get("connectTimeout") or timeout or DEFAULT_CONNECT_TIMEOUT
        )
        read_timeout = (
            runtime_option.get("readTimeout") or timeout or DEFAULT_READ_TIMEOUT
        )
        ...
        timeout = aiohttp.ClientTimeout(
            sock_read=read_timeout, sock_connect=connect_timeout
        )
    ...
    def do_action(request: TeaRequest, runtime_option=None) -> TeaResponse:
        ...
        if verify:
            verify = (
                runtime_option.get("ca", True)
                if runtime_option.get("ca", True) is not None
                else True
            )
    ...
    def allow_retry(dic, retry_times, now=None) -> bool:
        ...
        if (
            ...
            or dic.get("retryable") is not True
            and retry_times >= 1
        ):
            return False
        else:
    ...
    def get_backoff_time(dic, retry_times) -> int:
        ...
        if not isinstance(back_off_time, int) and not (
            isinstance(back_off_time, str) and back_off_time.isdigit()
        ):
            return default_back_off_time
