...
class TeaException(Exception):
    def __init__(self, dic):
        ...
        if (
            isinstance(dic.get("data"), dict)
            and dic.get("data").get("statusCode") is not None
        ):
...
class UnretryableException(TeaException):
    def __init__(self, request: TeaRequest, ex: Exception):
        self.last_request = request
        self.inner_exception = ex
        if isinstance(ex, TeaException):
            super().__init__({"code": ex.code, "message": ex.message, "data": ex.data})
        else:
            super().__init__(
                {
                    "message": repr(ex),
