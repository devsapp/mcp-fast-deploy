...
class CLIProfileCredentialsProvider(ICredentialsProvider):
    ...
    def get_credentials(self) -> Credentials:
        ...
        if self._should_reload_credentials_provider():
            if not os.path.exists(self._profile_file) or not os.path.isfile(
                self._profile_file
            ):
                raise CredentialException(
                    f"unable to open credentials file: {self._profile_file}"
                )
            try:
    ...
    def _get_credentials_provider(
        self, config: Dict, profile_name: str
        ...
        if not profiles:
            raise CredentialException(
                f"unable to get profile with '{profile_name}' form cli credentials file."
            )
        ...
        for profile in profiles:
            if profile.get("name") is not None and profile["name"] == profile_name:
                ...
                if mode == "AK":
                    ...
                    previous_provider = self._get_credentials_provider(
                        config, profile.get("source_profile")
                    )
                    return RamRoleArnCredentialsProvider(
                    ...
                    raise CredentialException(
                        f"unsupported profile mode '{mode}' form cli credentials file."
                    )
