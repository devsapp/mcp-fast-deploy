...
class EcsRamRoleCredentialsProvider(ICredentialsProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self._role_name = (
            role_name if role_name is not None else au.environment_ecs_metadata
        )
        self._disable_imds_v1 = (
            disable_imds_v1
            if disable_imds_v1 is not None
            else au.environment_imds_v1_disabled.lower() == "true"
        )
        ...
        self._runtime_options = {
            "connectTimeout": (
                ...
                else EcsRamRoleCredentialsProvider.DEFAULT_CONNECT_TIMEOUT
            ),
            "readTimeout": (
                self._http_options.read_timeout
                if self._http_options.read_timeout is not None
                else EcsRamRoleCredentialsProvider.DEFAULT_READ_TIMEOUT
            ),
        ...
        if async_update_enabled:
            ...
            def refresh_task():
                if self._should_refresh:
                    log.debug(
                        f"Begin checking or refreshing credentials asynchronously"
                    )
