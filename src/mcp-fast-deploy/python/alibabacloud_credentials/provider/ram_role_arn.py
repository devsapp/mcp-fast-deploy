        ...
        *,
        ...
        if self._role_session_name is None or self._role_session_name == "":
            self._role_session_name = (
                f"credentials-python-{str(int(time.mktime(time.localtime())))}"
            )
        ...
        if self._role_arn is None or self._role_arn == "":
            raise ValueError(
                "role_arn or environment variable ALIBABA_CLOUD_ROLE_ARN cannot be empty"
            )
        ...
        if sts_endpoint is not None and sts_endpoint != "":
            ...
            if enable_vpc is not None:
                ...
                prefix = (
                    "sts-vpc" if au.environment_enable_vpc.lower() == "true" else "sts"
                )
            if sts_region_id is not None and sts_region_id != "":
                self._sts_endpoint = f"{prefix}.{sts_region_id}.aliyuncs.com"
            elif (
                au.environment_sts_region is not None
                and au.environment_sts_region != ""
            ):
                self._sts_endpoint = (
                    f"{prefix}.{au.environment_sts_region}.aliyuncs.com"
                )
            else:
        ...
        self._runtime_options = {
            "connectTimeout": (
                self._http_options.connect_timeout
                if self._http_options.connect_timeout is not None
                else RamRoleArnCredentialsProvider.DEFAULT_CONNECT_TIMEOUT
            ),
            "readTimeout": (
                self._http_options.read_timeout
                if self._http_options.read_timeout is not None
                else RamRoleArnCredentialsProvider.DEFAULT_READ_TIMEOUT
            ),
    ...
    def _refresh_credentials(self) -> RefreshResult[Credentials]:
        ...
        if pre_credentials is None:
            raise CredentialException(
                "unable to load original credentials from the provider in RAM role arn"
            )
        ...
        if (
            "AccessKeyId" not in cre
            or "AccessKeySecret" not in cre
            or "SecurityToken" not in cre
        ):
            raise CredentialException(
                f'error retrieving credentials from ram_role_arn result: {response.body.decode("utf-8")}'
            )
