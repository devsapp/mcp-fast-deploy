...
EXECUTOR = ThreadPoolExecutor(
    max_workers=INT64_MAX, thread_name_prefix="non-blocking-refresh"
)
...
class RefreshResult(Generic[T]):
    def __init__(
        self, *, value: T, stale_time: int = INT64_MAX, prefetch_time: int = INT64_MAX
    ):
        self._value = value
...
class NonBlocking(PrefetchStrategy):
    ...
    def prefetch(self, action: Callable):
        if not CONCURRENT_REFRESH_LEASES.acquire(False):
            log.warning(
                "Skipping a background refresh task because there are too many other tasks running."
            )
            return
        ...
        try:
            ...
            log.warning(
                f"Exception occurred when submitting background task.", exc_info=True
            )
        finally:
...
class RefreshCachedSupplier(Generic[T]):
    ...
    def __init__(
        self,
        refresh_callable: Callable[[], RefreshResult[T]],
    ...
    def _sync_call(self) -> T:
        if self._cache_is_stale():
            ...
            log.debug(
                f"Prefetching using strategy: {self._prefetch_strategy.__class__.__name__}"
            )
    ...
    def _refresh_cache(self):
        acquired = self._refresh_lock.acquire(
            timeout=RefreshCachedSupplier.REFRESH_BLOCKING_MAX_WAIT
        )
        try:
            if self._cache_is_stale() or self._should_initiate_cache_prefetch():
                try:
                    self._cached_value = self._handle_fetched_success(
    ...
    async def _refresh_cache_async(self):
        ...
        try:
            if self._cache_is_stale() or self._should_initiate_cache_prefetch():
                try:
                    self._cached_value = self._handle_fetched_success(
                        await self._refresh_callable_async()
                    )
                except Exception as ex:
                    self._cached_value = self._handle_fetched_failure(ex)
        finally:
    ...
    def _handle_fetched_success(self, value: RefreshResult[T]) -> RefreshResult[T]:
        log.debug(
            f"Refresh credentials successfully, retrieved value is {value}, cached value is {self._cached_value}"
        )
        self._consecutive_refresh_failures = 0
        ...
        if now < value.stale_time() + RefreshCachedSupplier.STALE_TIME:
            ...
            return RefreshResult(
                value=value.value(), stale_time=now, prefetch_time=value.prefetch_time()
            )
        ...
        if self._cached_value is None:
            ...
            return RefreshResult(
                value=self._cached_value.value(),
                stale_time=now + 1,
                prefetch_time=self._cached_value.prefetch_time(),
            )
        else:  # ALLOW
            extended_stale_time = now + int(
                (50 * 1000 + random.randint(0, 20 * 1000 + 1)) / 1000
            )
            log.warning(
            ...
            return RefreshResult(
                value=self._cached_value.value(),
                stale_time=extended_stale_time,
                prefetch_time=self._cached_value.prefetch_time(),
            )

    def _handle_fetched_failure(self, exception: Exception) -> RefreshResult[T]:
        log.warning(
            f"Refresh credentials failed, cached value is {self._cached_value}, error: {exception}"
        )
        ...
        if self._stale_value_behavior == StaleValueBehavior.STRICT:
            ...
            new_stale_time = int(
                _jitter_time(
                    now * 1000,
                    1000,
                    _max_stale_failure_jitter(self._consecutive_refresh_failures),
                )
                / 1000
            )
            log.warning(
