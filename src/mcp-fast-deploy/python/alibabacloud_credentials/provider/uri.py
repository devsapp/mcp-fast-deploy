...
class URLCredentialsProvider(ICredentialsProvider):
    ...
    def __init__(
        ...
        *,
        ...
        if self._uri is None or self._uri == "":
            raise ValueError(
                "uri or environment variable ALIBABA_CLOUD_CREDENTIALS_URI cannot be empty"
            )
        ...
        self._runtime_options = {
            "connectTimeout": (
                ...
                else URLCredentialsProvider.DEFAULT_CONNECT_TIMEOUT
            ),
            "readTimeout": (
                self._http_options.read_timeout
                if self._http_options.read_timeout is not None
                else URLCredentialsProvider.DEFAULT_READ_TIMEOUT
            ),
    ...
    def _refresh_credentials(self) -> RefreshResult[Credentials]:
        ...
        if (
            content_code != "Success"
            or "AccessKeyId" not in dic
            or "AccessKeySecret" not in dic
            or "SecurityToken" not in dic
            or "Expiration" not in dic
        ):
            raise CredentialException(
