    ...
    def __init__(self, config=None):
        if isinstance(config, Config):
            ...
            self.connect_timeout = (
                config.connect_timeout or AlibabaCloudCredentialsProvider.timeout
            )
    ...
    def _verify_empty_args(self, *args, config):
        if None in args and config is None:
            raise CredentialException(
                '"%s" needs to receive a "model.Config" object or other necessary args'
                % self.__class__
            )
...
class DefaultCredentialsProvider(AlibabaCloudCredentialsProvider):
    def __init__(self):
        super().__init__()
        self.user_configuration_providers = [EnvironmentVariableCredentialsProvider()]
        if au.enable_oidc_credential:
            self.user_configuration_providers.append(
        ...
        if role_name is not None:
            self.user_configuration_providers.append(
                EcsRamRoleCredentialProvider(role_name)
            )
...
class EcsRamRoleCredentialProvider(AlibabaCloudCredentialsProvider):
    ...
    def __init__(self, role_name=None, config=None):
        ...
        self.__ecs_metadata_fetch_error_msg = (
        ...
        self.disable_imds_v1 = (
            au.environment_imds_v1_disabled
            and au.environment_imds_v1_disabled.lower() == "true"
        )

        if isinstance(config, Config):
            self.disable_imds_v1 = (
                config.disable_imds_v1 is not None and config.disable_imds_v1 == True
            )
    ...
    def _get_metadata_token(self, url=None):
        ...
        tea_request.headers["X-aliyun-ecs-metadata-token-ttl-seconds"] = str(
            self.default_metadata_token_duration
        )
        if not url:
    ...
    def __init__(
        ...
        if self.region_id is not None:
            ...
            self._set_arg(
                ...
                (
                    "sts.aliyuncs.com"
                    if config is None or config.sts_endpoint is None
                    else config.sts_endpoint
                ),
            )
        ...
        if response.status_code == 200:
            ...
            if "Credentials" in dic:
                ...
                return credentials.RamRoleArnCredential(
                    cre.get("AccessKeyId"),
                    cre.get("AccessKeySecret"),
                    cre.get("SecurityToken"),
                    expiration,
                    self,
                )
...
class OIDCRoleArnCredentialProvider(AlibabaCloudCredentialsProvider):
    ...
    def __init__(
        ...
        self._verify_empty_args(
            role_arn, oidc_provider_arn, oidc_token_file_path, config=config
        )
        super().__init__(config)
        self._set_arg("role_arn", role_arn)
...
class RsaKeyPairCredentialProvider(AlibabaCloudCredentialsProvider):
    ...
    def __init__(
        self, access_key_id=None, access_key_secret=None, region_id=None, config=None
    ):
        self._verify_empty_args(access_key_id, access_key_secret, config=config)
        super().__init__(config)
        self._set_arg("access_key_id", access_key_id)
...
class ProfileCredentialsProvider(AlibabaCloudCredentialsProvider):
    ...
    def parse_ini(self):
        file_path = (
            self.file_path if self.file_path else au.environment_credentials_file
        )
    ...
    def _create_credential(self, config):
        ...
        if not config_type:
            ...
            return self._get_sts_get_session_access_key_provider(
                config
            ).get_credentials()
    ...
    def _get_sts_get_session_access_key_provider(config):
        ...
        if not public_key_id or not private_key:
            raise CredentialException(
                "The configured public_key_id or private_key_file content is empty"
            )
...
class EnvironmentVariableCredentialsProvider(AlibabaCloudCredentialsProvider):
    def get_credentials(self):
        ...
        if security_token is not None and len(security_token) > 0:
            return credentials.StsCredential(
