...
environment_ecs_metadata_disabled = os.environ.get(
    "ALIBABA_CLOUD_ECS_METADATA_DISABLED", "false"
)
...
environment_cli_profile_disabled = os.environ.get(
    "ALIBABA_CLOUD_CLI_PROFILE_DISABLED", "false"
)
...
enable_oidc_credential = (
    environment_oidc_token_file is not None
    and environment_oidc_token_file != ""
    and environment_role_arn is not None
    and environment_role_arn != ""
    and environment_oidc_provider_arn is not None
    and environment_oidc_provider_arn != ""
)
private_key = None
...
def get_home():
    if platform.system() == "Windows":
        ...
        if home:
            ...
            return (
                home_path
                if has_drive_in_home_path
                else os.path.join(home_drive or "", home_path)
            )
        else:
