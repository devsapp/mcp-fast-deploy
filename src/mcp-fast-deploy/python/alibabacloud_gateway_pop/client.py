...
class Client(SPIClient):
    ...
    def modify_request(
        ...
        signature_algorithm = UtilClient.default_string(
            request.signature_algorithm, self._sha_256
        )
        hashed_request_payload = Encoder.hex_encode(
            Encoder.hash(UtilClient.to_bytes(""), signature_algorithm)
        )
        if not UtilClient.is_unset(request.stream):
            tmp = UtilClient.read_as_bytes(request.stream)
            hashed_request_payload = Encoder.hex_encode(
                Encoder.hash(tmp, signature_algorithm)
            )
            request.stream = tmp
            ...
            if not UtilClient.is_unset(request.body):
                if UtilClient.equal_string(request.req_body_type, "json"):
                    ...
                    hashed_request_payload = Encoder.hex_encode(
                        Encoder.hash(UtilClient.to_bytes(json_obj), signature_algorithm)
                    )
                    ...
                    hashed_request_payload = Encoder.hex_encode(
                        Encoder.hash(UtilClient.to_bytes(form_obj), signature_algorithm)
                    )
        ...
        if not UtilClient.equal_string(request.auth_type, "Anonymous"):
            ...
            if UtilClient.equal_string(auth_type, "bearer"):
                ...
                region = self.get_region(
                    request.product_id, config.endpoint, config.region_id
                )
                signingkey = self.get_signingkey(
                    signature_algorithm,
                    access_key_secret,
                    request.product_id,
                    region,
    ...
    async def modify_request_async(
        ...
        if not UtilClient.equal_string(request.auth_type, "Anonymous"):
            ...
            if UtilClient.equal_string(auth_type, "bearer"):
                ...
                signingkey = await self.get_signingkey_async(
                    signature_algorithm,
                    access_key_secret,
                    request.product_id,
                    region,
            ...
            raise TeaException(
                {
                    ...
                    "accessDeniedDetail": self.default_any(
                        err.get("AccessDeniedDetail"), err.get("accessDeniedDetail")
                    ),
                }
            )
    ...
    def get_authorization(
        ...
        signature = self.get_signature(
            pathname, method, query, headers, signature_algorithm, payload, signingkey
        )
    ...
    async def get_authorization_async(
        ...
        signature = await self.get_signature_async(
            pathname, method, query, headers, signature_algorithm, payload, signingkey
        )
    ...
    def get_signature(
        ...
        hex = Encoder.hex_encode(
            Encoder.hash(UtilClient.to_bytes(string_to_sign), signature_algorithm)
        )
    ...
    def get_region(
        ...
        without_port = strs[0]
        pre_region = StringClient.replace(
            without_port, f".{self._endpoint_suffix}", "", None
        )
    ...
    def build_canonicalized_resource(
        ...
        if not UtilClient.is_unset(query):
            ...
            for key in sorted_query_array:
                canonicalized_resource = (
                    f"{canonicalized_resource}{separator}{Encoder.percent_encode(key)}"
                )
    ...
    def get_signed_headers(
        ...
        for key in sorted_headers_array:
            ...
            if (
                StringClient.has_prefix(lower_key, "x-acs-")
