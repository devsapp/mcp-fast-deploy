...
class Client(SPIClient):
    ...
    def modify_configuration(
        ...
        config.endpoint = self.get_endpoint(
            config.region_id, config.network, config.endpoint
        )
    ...
    async def modify_configuration_async(
        ...
        config = context.configuration
        config.endpoint = await self.get_endpoint_async(
            config.region_id, config.network, config.endpoint
        )
    ...
    def modify_request(
        ...
        final_compress_type = self.get_final_request_compress_type(
        ...
        if not UtilClient.is_unset(request.body):
            if StringClient.equals(
                request.req_body_type, "json"
            ) or StringClient.equals(request.req_body_type, "formData"):
        ...
        if StringClient.equals(signature_version, "v4"):
            ...
            request.headers["authorization"] = self.get_authorization_v4(
                context, date, content_hash, access_key_id, access_key_secret
            )
            return
    ...
    async def modify_request_async(
        ...
        if not UtilClient.empty(security_token):
            request.headers["x-acs-security-token"] = security_token
        signature_version = UtilClient.default_string(request.signature_version, "v1")
        final_compress_type = await self.get_final_request_compress_type_async(
            request.action, request.headers
        )
        content_hash = ""
        # get body bytes
        ...
        if not UtilClient.is_unset(request.body):
            if not UtilClient.empty(final_compress_type):
                compressed = await SLS_UtilClient.compress_async(
                    body_bytes, final_compress_type
                )
                body_bytes = compressed
            content_hash = await self.make_content_hash_async(
                body_bytes, signature_version
            )
            request.stream = body_bytes
        ...
        if StringClient.equals(signature_version, "v4"):
            ...
            request.headers["x-log-date"] = date
            request.headers["x-log-content-sha256"] = content_hash
            request.headers["authorization"] = await self.get_authorization_v4_async(
                context, date, content_hash, access_key_id, access_key_secret
            )
            return
        if not UtilClient.empty(access_key_id):
        ...
        request.headers["date"] = UtilClient.get_date_utcstring()
        request.headers["content-md5"] = content_hash
        request.headers["authorization"] = await self.get_authorization_async(
            request.pathname,
            request.method,
            request.query,
            request.headers,
    ...
    def make_content_hash(
        ...
        if StringClient.equals(signature_version, "v4"):
            if UtilClient.is_unset(content) or UtilClient.equal_string(
                f"{SLS_UtilClient.bytes_length(content)}", "0"
            ):
                return ""
            return StringClient.to_lower(
                Encoder.hex_encode(Encoder.hash(content, "SLS4-HMAC-SHA256"))
            )
        return StringClient.to_upper(
            Encoder.hex_encode(Signer.md5sign_for_bytes(content))
        )
    ...
    def modify_response(
        ...
        if UtilClient.is_4xx(response.status_code) or UtilClient.is_5xx(
        ...
        if not UtilClient.is_unset(response.body):
            ...
            if not UtilClient.is_unset(bodyraw_size) and not UtilClient.is_unset(
                compress_type
            ):
                uncompressed_data = SLS_UtilClient.read_and_uncompress_block(
                    response.body, compress_type, bodyraw_size
                )
            if UtilClient.equal_string(request.body_type, "binary"):
                ...
                response.deserialized_body = UtilClient.read_as_string(
                    uncompressed_data
                )
    ...
    async def modify_response_async(
        ...
        if not UtilClient.is_unset(response.body):
            ...
            if not UtilClient.is_unset(bodyraw_size) and not UtilClient.is_unset(
                ...
                uncompressed_data = (
                    await SLS_UtilClient.read_and_uncompress_block_async(
                        response.body, compress_type, bodyraw_size
                    )
                )
            if UtilClient.equal_string(request.body_type, "binary"):
                ...
                response.deserialized_body = await UtilClient.read_as_string_async(
                    uncompressed_data
                )
            elif UtilClient.equal_string(request.body_type, "json"):
                ...
                response.deserialized_body = await UtilClient.read_as_json_async(
                    uncompressed_data
                )
            else:
                response.deserialized_body = await UtilClient.read_as_string_async(
    ...
    def get_endpoint(
        ...
        if not UtilClient.empty(network):
            if StringClient.equals(network, "intranet"):
                ...
                if StringClient.equals(
                    region_id, "cn-hangzhou-corp"
                ) or StringClient.equals(region_id, "cn-shanghai-corp"):
    ...
    def get_signature(
        ...
        return Encoder.base_64encode_to_string(
            Signer.hmac_sha256sign(string_to_sign, secret)
        )
    ...
    async def get_signature_async(
        ...
        resource = pathname
        string_to_sign = ""
        canonicalized_resource = await self.build_canonicalized_resource_async(
            resource, query
        )
        ...
        if not UtilClient.is_unset(query):
            ...
            for param_name in sorted_params:
                canonicalized_resource = (
                    f"{canonicalized_resource}{separator}{param_name}"
                )
    ...
    def build_canonicalized_headers(
        ...
        for header in sorted_headers:
            if StringClient.contains(
                StringClient.to_lower(header), "x-log-"
            ) or StringClient.contains(StringClient.to_lower(header), "x-acs-"):
                canonicalized_headers = (
                    f"{canonicalized_headers}{header}:{headers.get(header)}\n"
                )
    ...
    def get_authorization_v4(
        ...
        signingkey = self.get_signingkey_v4(
            "SLS4-HMAC-SHA256", access_key_secret, region, date_short
        )
        signature = self.get_signature_v4(
            context,
            "SLS4-HMAC-SHA256",
            header_str,
            date,
    ...
    async def get_authorization_v4_async(
        ...
        # for fix php sdk bug
        scope = f"{date_short}/{region}/sls/aliyun_v4_request"
        signingkey = await self.get_signingkey_v4_async(
            "SLS4-HMAC-SHA256", access_key_secret, region, date_short
        )
        signature = await self.get_signature_v4_async(
            context,
            "SLS4-HMAC-SHA256",
            header_str,
            date,
    ...
    def get_signature_v4(
        ...
        headers = self.build_canonicalized_headers_v4(
            request.headers, signed_header_str
        )
        string_to_hash = f"{request.method}\n{canonical_uri}\n{resources}\n{headers}\n{signed_header_str}\n{content_sha_256}"
        hex = Encoder.hex_encode(
            Encoder.hash(UtilClient.to_bytes(string_to_hash), signature_algorithm)
        )
    ...
    async def get_signature_v4_async(
        ...
        headers = await self.build_canonicalized_headers_v4_async(
            request.headers, signed_header_str
        )
        string_to_hash = f"{request.method}\n{canonical_uri}\n{resources}\n{headers}\n{signed_header_str}\n{content_sha_256}"
        hex = Encoder.hex_encode(
    ...
    def get_signed_header_str_v4(
        ...
        for key in sorted_headers_array:
            ...
            if (
                StringClient.has_prefix(lower_key, "x-log-")
                or StringClient.has_prefix(lower_key, "x-acs-")
                or StringClient.equals(lower_key, "host")
                or StringClient.equals(lower_key, "content-type")
            ):
    ...
    def get_region(
        ...
        config = context.configuration
        if not UtilClient.is_unset(config.region_id) and not UtilClient.empty(
            config.region_id
        ):
