...
def lz4_compress(data: bytes) -> bytes:
    try:
        ...
        raise Exception(f"fail to compress using compresstype lz4, error: {e}")
...
def zstd_compress(data: bytes) -> bytes:
    try:
        ...
        raise Exception(f"fail to compress using compresstype zstd, error: {e}")
...
def gzip_compress(data: bytes) -> bytes:
    try:
        ...
        raise Exception(f"fail to compress using compresstype gzip, error: {e}")
...
def lz4_decompress(data: bytes, raw_size: int) -> bytes:
    try:
        ...
        raise Exception(f"fail to decompress using compresstype lz4, error: {e}")
...
def zstd_decompress(data: bytes, raw_size: int) -> bytes:
    try:
        ...
        raise Exception(f"fail to decompress using compresstype zstd, error: {e}")
...
def gzip_decompress(data: bytes, raw_size: int) -> bytes:
    try:
        ...
        raise Exception(f"fail to decompress using compresstype gzip, error: {e}")
...
class Client:
    ...
    def read_and_uncompress_block(
        ...
        if len(decompressed) != raw_size:
            raise Exception(
                "unexpected uncompressed size: %d, expected: %d, compressType: %s".format(
                    len(decompressed), raw_size, compress_type
                )
            )
